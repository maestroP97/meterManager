<?php
    ...
    if(isset($readData))
    {
        $data ='<table class="table table-hover table-condensed" id="tableEntreprise">
                    ...
                    <tbody>';
        // $statut = "Archivé";
        $docList = "SELECT d.id, d.Nom, d.Type, d.Secteur, d.Localisation, d.Telephone, d.CreateAt, u.UserName FROM users u, entreprises d WHERE d.UserId = u.id ;";
        // if($filter=="Archivé")
        // {
        ...
        if(mysqli_num_rows($result)>0)
        {
            while($doc =  mysqli_fetch_array($result))
            {
                $data .="<tr id='".$doc['id']."'>
                        ...
                        <td width='16%'>
                            <i onclick='editItem(".$doc["id"].")' data-toggle='modal' data-target='#accountModal' data-placement='top' title='Modifier cette entreprise' class='btn btn-info fas fa-pen' style='font-size:11px;'></i>
    ...
    elseif(isset($operation) && isset($Nom) && !empty($Type))
    {
        ...
        if($operation=="add"){
            $cmd = "SELECT id FROM entreprises WHERE Nom = '$Nom' AND Localisation='$Localisation' AND Type='$Type' AND Secteur='$Secteur' AND Telephone='$Telephone'";
            $result = mysqli_query($con,$cmd);
            if(mysqli_num_rows($result)<=0)
            {
                
                $cmd = "INSERT INTO entreprises (Nom, Localisation, Type, Secteur, Telephone, UserId) VALUES ('$Nom','$Localisation','$Type','$Secteur','$Telephone','$idUser')";
        ...
        {
            $cmd = "SELECT id FROM entreprises WHERE Nom = '$Nom' AND Localisation='$Localisation' AND Type='$Type' AND Secteur='$Secteur' AND Telephone='$Telephone' AND id<>'$idEdit'";
            $result = mysqli_query($con,$cmd);
            if(mysqli_num_rows($result)<=1)
            {
                $cmd = "UPDATE entreprises SET Nom='$Nom', Localisation='$Localisation', Type='$Type', Secteur='$Secteur', Telephone='$Telephone' WHERE id='$idEdit'";
                if(mysqli_query($con,$cmd)){
